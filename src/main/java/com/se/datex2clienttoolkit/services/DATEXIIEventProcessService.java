...
/**
 * 
 * This service processes Event DATEX II v2 messages (D2LogicalModel). The
 * payloads are inserted into the Event data store.
 * 
	...
	public void processClearedEvents() {
		synchronized (eventDataStore) {
			for (EventData eventData : eventDataStore.getAllEventData()) {
				...
				if (isClear && new Date().getTime() > publicationTime.getTime()
						+ expireClearedEventsAfterMins * 60 * 1000) {

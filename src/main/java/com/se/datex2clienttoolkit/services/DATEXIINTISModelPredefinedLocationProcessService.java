...
/**
 * 
 * This service processes PredefinedLocation DATEX II v2 messages
 * (D2LogicalModel). The payloads are inserted into the NwkLinkStatic,
 * NwkNodeStatic, NwkShapeStatic, ANPRRouteStatic, HATRISSectionStatic and
 * AlternateRouteStatic data stores.
 * 
...
public class DATEXIINTISModelPredefinedLocationProcessService extends DATEXIIProcessService {
	...
	@Autowired
	public DATEXIINTISModelPredefinedLocationProcessService(LinkShapeStaticDataStore linkShapeStaticDataStore,
			NwkLinkStaticDataStore nwkLinkStaticDataStore, ANPRRouteStaticDataStore anprRouteStaticDataStore,
			HATRISSectionStaticDataStore hatrisSectionStaticDataStore, NwkNodeStaticDataStore nwkNodeStaticDataStore,
			AlternateRouteStaticDataStore alternateRouteStaticDataStore) {
		super();
	...
	public void processMessage(D2LogicalModel d2LogicalModel) {
		...
		PredefinedLocationsPublication predefinedLocationsPublication = (PredefinedLocationsPublication) d2LogicalModel
		...
		if (predefinedLocationsPublication != null) {
			...
			List<PredefinedLocationContainer> predefinedLocationContainerList = predefinedLocationsPublication
					.getPredefinedLocationContainer();
	...
	private void processPredefinedLocationContainer(PredefinedLocationContainer predefinedLocationContainer,
		...
		if (predefinedLocationContainer instanceof PredefinedItinerary) {
			...
			List<PredefinedItineraryIndexPredefinedLocation> predefinedLocationList = predefinedItinerary
			...
			while (iterator.hasNext()) {
				...
				processPredefinedLocation(predefinedItineraryIndexPredefinedLocation.getPredefinedLocation(),
						publicationTime, predefinedLocationContainerId);
			}
			...
			log.error("Unexpected data type for PredefinedLocationContainer: "
					+ predefinedLocationContainer.getClass().getSimpleName());
			return;
		}
	}

	private void processPredefinedLocation(PredefinedLocation predefinedLocation, Date publicationTime,
			String predefinedLocationContainerId) {
		...
		if (predefinedLocationContainerId.equals("NTIS_Network_Links")) {
			NwkLinkStaticData nwkLinkStaticData = new NwkLinkStaticData(predefinedLocationIdentifier, publicationTime,
					predefinedLocation);
			nwkLinkStaticDataStore.updateData(nwkLinkStaticData);
		} else if (predefinedLocationContainerId.equals("NTIS_Network_Nodes")) {
			NwkNodeStaticData nwkNodeStaticData = new NwkNodeStaticData(predefinedLocationIdentifier, publicationTime,
					predefinedLocation);
			nwkNodeStaticDataStore.updateData(nwkNodeStaticData);
		} else if (predefinedLocationContainerId.startsWith("NTIS_Link_Shape_")) {
			LinkShapeStaticData linkShapeStaticData = new LinkShapeStaticData(predefinedLocationIdentifier,
					publicationTime, predefinedLocation);
			linkShapeStaticDataStore.updateData(linkShapeStaticData);
		} else if (predefinedLocationContainerId.startsWith("NTIS_ANPR_Route_")) {
			ANPRRouteStaticData anprRouteStaticData = new ANPRRouteStaticData(predefinedLocationIdentifier,
					publicationTime, predefinedLocation);
			...
			HATRISSectionStaticData hatrisSectionStaticData = new HATRISSectionStaticData(predefinedLocationIdentifier,
					publicationTime, predefinedLocation);
			...
			AlternateRouteStaticData alternateRouteStaticData = new AlternateRouteStaticData(
					predefinedLocationIdentifier, publicationTime, predefinedLocation);

...
/**
 * 
 * This service processes MeasurementSites DATEX II v2 messages
 * (D2LogicalModel). The payloads are inserted into the TAME, MIDAS, ANPR and
 * TMU data stores.
 * 
...
public class DATEXIINTISModelMeasurementSitesProcessService extends DATEXIIProcessService {
	...
	@Autowired
	public DATEXIINTISModelMeasurementSitesProcessService(TAMEStaticDataStore tameStaticDataStore,
			MIDASStaticDataStore midasStaticDataStore, ANPRStaticDataStore anprStaticDataStore,
	...
	public void processMessage(D2LogicalModel d2LogicalModel) {
		...
		MeasurementSiteTablePublication measurementSiteTablePublication = (MeasurementSiteTablePublication) d2LogicalModel
		...
		if (measurementSiteTablePublication != null) {
			...
			List<MeasurementSiteTable> measurementSiteTableList = measurementSiteTablePublication
					.getMeasurementSiteTable();
	...
	private void processMeasurementSiteRecord(MeasurementSiteRecord measurementSiteRecord, Date publicationTime,
			String measurementSiteTableId) {
		...
		case "NTIS_TAME_Measurement_Sites":
			TAMEStaticData tameStaticData = new TAMEStaticData(measurementSiteRecordIdentifier, publicationTime,
					measurementSiteRecord);
			tameStaticDataStore.updateData(tameStaticData);
			break;
		case "NTIS_MIDAS_Measurement_Sites":
			MIDASStaticData midasStaticData = new MIDASStaticData(measurementSiteRecordIdentifier, publicationTime,
					measurementSiteRecord);
			midasStaticDataStore.updateData(midasStaticData);
			break;
		case "NTIS_ANPR_Measurement_Sites":
			ANPRStaticData anprStaticData = new ANPRStaticData(measurementSiteRecordIdentifier, publicationTime,
					measurementSiteRecord);
			anprStaticDataStore.updateData(anprStaticData);
			break;
		case "NTIS_TMU_Measurement_Sites":
			TMUStaticData tmuStaticData = new TMUStaticData(measurementSiteRecordIdentifier, publicationTime,
					measurementSiteRecord);
